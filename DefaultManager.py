...
class DefaultManager():
    ...
    logger = Logger.Logger(
        ...
        loglevel=1,
        logger="DefaultManager.py").getlog()
    # 网站首页
    ...
    def dispatch(self):
        ...
        # target=self.spider_basic, name='basic_thread')
        # 爬取商城详情的线程
        store_details_thread = threading.Thread(
            target=self.spider_store_details, name='store_details_thread')
        # 爬取产品详情线程
        product_details_thread = threading.Thread(
            target=self.spider_product_details, name='product_details_thread')
        # basic_thread.start()
        store_details_thread.start()
        # product_details_thread.start()
        # basic_thread.join()
        store_details_thread.join()
        # product_details_thread.join()
